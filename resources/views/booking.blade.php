    ...
    <div class="bg-white-100 text-black/100 h-screen w-full flex flex-col">
        ...
        <main class="flex-1 mt-16">
            ...
            <section class="relative bg-[#0f1021] text-white py-10 mx-auto text-center">
                ...
                @if (session('error'))
                    <div class="bg-red-200 border border-red-400 text-red-700 px-4 py-3 rounded relative max-w-[672px]"
                        role="alert">
                ...
                @if (session('success'))
                    <div class="bg-green-600 border border-green-600 text-white px-4 py-3 mb-5 rounded relative max-w-[672px] mx-auto"
                        role="alert">
                        ...
                        <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
                            <svg class="fill-current h-6 w-6 text-green-500" role="button"
                                ...
                                <title>Close</title>
                                <path fill-rule="evenodd"
                                    d="M14.348 5.652a1.2 1.2 0 0 0-1.697 0L10 8.183l-2.651-2.531a1.2 1.2 0 1 0-1.697 1.697L8.303 10l-2.651 2.53a1.2 1.2 0 0 0 1.697 1.697L10 11.817l2.651 2.531a1.2 1.2 0 0 0 1.697-1.697L11.697 10l2.651-2.53a1.2 1.2 0 0 0 0-1.697z"
                                    clip-rule="evenodd" />
                            </svg>
                        </span>
                    </div>
                ...
                @forelse ($bookings as $booking)
                                    ...
                                    <div class="flex flex-col justify-between p-6 md:w-2/3 text-left">
                                        <div>
                                            <h5 class="mb-4 text-2xl font-bold tracking-tight text-gray-900">
                                                {{ $booking->car->carModel->brand }} - {{ $booking->car->carModel->model_name }}
                                                {{ $booking->car->carModel->model_desc }}
                                            </h5>
                                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-gray-700 text-sm">
                                                <p><span class="font-semibold">Car Type:</span> {{ $booking->car->carModel->car_type }}
                                                </p>
                                                <p><span class="font-semibold">Transmission:</span>
                                                    {{ $booking->car->carModel->transmission }}</p>
                                                <p><span class="font-semibold">Created Date:</span><br>
                                                    {{ \Carbon\Carbon::parse($booking->created_at)->format('Y-m-d') }}</p>
                                                <p><span class="font-semibold">Amount Due:</span><br> ₱{{ $booking->amount_due }}</p>
                                                <p><span class="font-semibold">Pickup Date:</span><br>
                                                    {{ \Carbon\Carbon::parse($booking->pickup_date)->format('Y-m-d') }}</p>
                                                <p><span class="font-semibold">Return Date:</span><br>
                                                    {{ \Carbon\Carbon::parse($booking->return_date)->format('Y-m-d') }}</p>
                                            </div>
                                        </div>
                                        <?php
                    ...
                    switch ($booking->latestStatus->status) {
                        ...
                        case 'For Pick-Up':
                            ...
                            break;
                        case 'Ongoing':
                            $statusColor = 'background-color: #16a34a; /* Green 600 */';
                            break;
                        case 'Due for Return':
                        ...
                        default:
                            ...
                            break;
                                        ...
                                        <div class="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                            <span
                                                class="inline-block px-4 py-2 text-sm font-semibold text-white text-center rounded-lg w-[100px]"
                                                style="{{ $statusColor }}">
                                                {{ $booking->latestStatus->status }}
                                            </span>
                                            @if (in_array($booking->latestStatus->status, ['Unpaid', 'Paid', 'Pending', 'Approved']))
                                                <div data-modal-target="cancel-modal-{{ $booking->id }}"
                                ...
                                <div id="cancel-modal-{{ $booking->id }}" tabindex="-1" aria-hidden="true"
                                    ...
                                    <div
                                        ...
                                        <!-- CANCELATION MODAL -->
                                        <form action="{{ route('cancel_booking', ['booking' => $booking->id]) }}"
                                            id="cancel-booking-form-{{ $booking->id }}" method="POST" class="space-y-4">
                                            @csrf
                                            ...
                                            <div class="px-6 py-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <div>
                                                    ...
                                                    <textarea id="cancel-reason-{{ $booking->id }}" rows="6" required
                                                        ...
                                                        name="cancel_reason" placeholder="Enter your reason here..."></textarea>
                                                </div>

                                                <!-- Suggested Reasons -->
                                                <div
                                                    class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg shadow-inner overflow-y-auto max-h-60">
                                                    <p class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3">Common
                                                        Reasons:</p>
                                                    <ul class="space-y-2">
                                                        <li>
                                                            <button
                                                                ...
                                                                data-target="cancel-reason-{{ $booking->id }}" type="button">
                                            ...
                                            <div class="px-6">
                                                <label class="flex items-center text-sm text-gray-200 mb-3">
                                                    ...
                                                    <span class="ms-2">I confirm that I understand and agree to proceed with the
                                                        cancellation of
                                                        my
                                                        booking.</span>
                                                </label>
